...
export async function GET(req: NextRequest) {
  ...
  const { data: hospital } = await supabaseAdmin
    .from("hospitals")
    .select("spiritual_support_enabled")
    .eq("id", ctx.hospitalId)
    .single();

  if (!hospital?.spiritual_support_enabled) {
    return NextResponse.json({ error: "Spiritual support is not enabled for this hospital" }, { status: 404 });
  }

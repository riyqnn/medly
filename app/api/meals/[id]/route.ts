...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const allowed = ["name", "description", "meal_type_tags", "is_available", "category_id", "image_url"];
    const updates: any = { updated_at: new Date().toISOString() };
    allowed.forEach(k => { if (body[k] !== undefined) updates[k] = body[k]; });
    const { data, error } = await supabase.from("meal_menus").update(updates)
      .eq("id", params.id).eq("hospital_id", hospitalId).is("deleted_at", null).select().single();
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase.from("meal_menus").update({ deleted_at: new Date().toISOString() })
    .eq("id", params.id).eq("hospital_id", hospitalId);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
